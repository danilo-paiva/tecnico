...
2. `composer install`
3. `composer start`
4. `composer test` → esperado: **OK (44 testes, 79 assertions)**
...
- Como comum, comprar em `compras.html` → funciona, só para si (combo travado;
  tentar outro id via API → `403 Voce so pode comprar ingressos para voce mesmo`).
