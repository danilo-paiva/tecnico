...
(function () {
  ...
  function displayUser() {
    ...
    const perfil = user.perfil || 'comum';
    const rotulo = nome ? nome + ' (' + perfil + ')' : '';

    const nameEls = document.querySelectorAll('#userName, #user-name, #nomeUsuario, #nome-usuario, [data-user-name], .user-name');
    nameEls.forEach(function (el) { el.textContent = rotulo; });
    ...
    if (singleName && !singleName.textContent) singleName.textContent = rotulo;
  }

  // Perfil do logado: 'administrador' tem escrita; 'comum' so lista.
  // Equivale a checagem do ValidateAdministrador no backend.
  function isAdmin() {
    try {
      var u = window.getUser ? window.getUser() : null;
      return !!u && (u.perfil || 'comum') === 'administrador';
