...
(function (global) {
  ...
  function authLogin(email, senha) {
    // Backend atual: POST /login com {participante:{email,senha}}.
    // Normaliza para {data:{token, usuario}} como o auth.js espera.
    return apiFetch('/login', {
      method: 'POST',
      body: { participante: { email: email, senha: senha } }
    }).then(function (res) {
      var d = (res && res.data) || {};
