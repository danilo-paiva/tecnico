...
Ou abrir `projeto.sln` no Visual Studio 2019 e pressionar F5.
...
- **Saída** (labels, apenas o que o enunciado pede):
  - Tempo de viagem estimado (dias e meses)
  - Δv total (km/s)
  - Razão de massa e combustível necessário (kg e toneladas)

> **Importante:** digitar os valores com **vírgula** decimal (`149,6`). Com ponto
> (`149.6`) o Windows pt-BR interpreta como 1496 — comportamento do `double.Parse`
> em cultura pt-BR. A interface avisa isso ao usuário.

## Lógica implementada (`MainWindow.xaml.cs`)

Constante gravitacional do Sol: **μ = 1.327 × 10¹¹ km³/s²**
(`double mu = 132700000000;` — distâncias convertidas de milhões de km para km).
...
Leitura da entrada no mesmo estilo do projeto do 2bi (sem validações):
...
| --- | --- |
| TextBox multilinha (`AcceptsReturn="True"`), leitura linha a linha, Labels, botões com evento `Click` | 03 / 04 |
| `Math.PI`, `Math.Sqrt`, `Math.Pow` (a³ e `e^x` via `Math.Pow(Math.E, x)`), `Math.Abs`, `Math.Truncate` | 05 |
...
- `e^x` calculado com `Math.Pow(Math.E, x)` (a aula 05 não lista `Math.Exp`);
...
- Dias de viagem **truncados** com `Math.Truncate` (258,84 → 258, igual ao PDF).
  No código há comentário com a linha alternativa para voltar ao arredondamento.

## Formatação de saída (aula 06)

| Saída | Técnica | Exemplo (pt-BR) |
| --- | --- | --- |
| Tempo de viagem | `String.Format("{0:0} dias (~{1:F1} meses)")` + `Math.Truncate` | `258 dias (~8,6 meses)` |
| Δv total (km/s) | `ToString("0.000")` + unidade | `5,591 km/s` |
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| Terra → Marte | 149,6 | 227,9 | 5000 | 310 | 258 dias (~8,6 meses) | 5,591 km/s | 26.436,07 kg (26,44 t) |
...
| Terra → Plutão | 149,6 | 5906,4 | 500 | 3000 | 16631 dias (~554,4 meses) | 15,499 km/s | 346,63 kg (0,35 t) |
